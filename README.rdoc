...
CDK maps these parameters to runtime environment variables such as <tt>FACEBOOK_ID</tt>, <tt>DB_URL</tt>, and <tt>TASK_PASSWORD</tt>. SecureString values such as <tt>{paramPath}ga_api_secret</tt> are not injected directly; CDK sets <tt>GA_API_SECRET_PARAM</tt> to the SSM parameter name, and Rails/MCP read it from SSM at runtime with decryption. CDK also sets <tt>GA_MEASUREMENT_ID</tt>.
